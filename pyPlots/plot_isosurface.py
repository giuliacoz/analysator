...
from mpl_toolkits.axes_grid1.inset_locator import inset_axes #giulia
...
def plot_isosurface(filename=None,
    ...
    :kword axes:        Provide the routine a set of axes to draw within instead of generating a new image.
                        It is recommended to either also provide cbaxes or activate nocb, unless one wants a colorbar to be automatically added next to the panel (but this may affect the overall layout)
    ...
    if (nooverwrite!=None and os.path.exists(savefigname)):
        ...
        if os.stat(savefigname).st_size > 0:
            print("Found existing file "+savefigname+". Not overwriting, returning and doing nothing.")
            return
        else:
    ...
    print('dimension surf_datamap:',np.ndim(surf_datamap))
    print('surf_datamap[0]:',surf_datamap[0])
    ...
    if color_var==None:
        ...
        print('dimension color_data:',np.ndim(color_data)) #giulia
        print('color_data[0]',color_data[0])
        color_data = color_data + 280 
        print('color_data[0] new',color_data[0])
        if vmin!=None:
            vminuse=vmin
        else: 
    ...
    maxrange = np.array([boxcoords[1]-boxcoords[0], boxcoords[3]-boxcoords[2], boxcoords[5]-boxcoords[4]]) / 2.0
    ...
    if ysize < 0.2*xsize: # 2.9D polar, perform rotation
        ...
        ax1.set_xlabel("Z ["+unitstr+"]", fontsize=fontsize3,weight='black')
        ax1.set_ylabel("X ["+unitstr+"]", fontsize=fontsize3,weight='black')
        ax1.set_zlabel("Y ["+unitstr+"]", fontsize=fontsize3,weight='black')
        ...
        generatedsurface = ax1.plot_trisurf(verts[:,0], verts[:,1], verts[:,2], triangles=faces, #giulia
                                            cmap=cmapuse, norm=norm, vmin=vminuse, vmax=vmaxuse, 
                                            lw=0.2, shade=True, edgecolors=None, antialiased=False, alpha=0.8 if transparent else 1.0)
        ax1.set_xlabel("X ["+unitstr+"]", fontsize=fontsize3,fontweight='black',labelpad=10)
        ax1.set_ylabel("Y ["+unitstr+"]", fontsize=fontsize3,fontweight='black',labelpad=7)
        #ax1.set_zlabel("Z ["+unitstr+"]", fontsize=fontsize3,fontweight='black',labelpad=2)
        ax1.set_zlabel('')
        # Set camera angle
        ax1.view_init(elev=angle[0], azim=angle[1])
        # Set virtual bounding box
        ...
        ax1.set_xlim([midvals[0]-maxrange[0], midvals[0]+maxrange[0]])
        ax1.set_ylim([midvals[1]-maxrange[1], midvals[1]+maxrange[1]])
        ax1.set_zlim([midvals[2]-maxrange[2], midvals[2]+maxrange[2]])
        ax1.tick_params(labelsize=fontsize3)#,width=1.5,length=3)
    ...
    if len(plot_title)!=0:
        ax1.set_title(plot_title,fontsize=fontsize2,fontweight='black')
    ...
    if not nocb:
        # First draw colorbar
        if usesci==0:        
            cb = fig.colorbar(generatedsurface,ticks=ticks, format=mtick.FuncFormatter(pt.plot.cbfmt), drawedges=False, fraction=0.023, pad=0.05)
        else:
            cb = fig.colorbar(generatedsurface,ticks=ticks,format=mtick.FuncFormatter(pt.plot.cbfmtsci),drawedges=False, fraction=0.023, pad=0.08) #giulia (fraction=0.046, pad=0.04)
        ...
        cb.ax.set_title(cb_title_use,fontsize=fontsize3*1.4)
